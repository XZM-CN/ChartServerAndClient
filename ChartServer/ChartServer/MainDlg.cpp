...
void CMainDlg::Innermain()
{
	...
	// AfxBeginThread(CChatServer::ServerListenThread,0);

	HANDLE hThread;
	DWORD dwThreadId;
	hThread=::CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE)CChatServer::ServerListenThread, m_pServerObj, 0, &dwThreadId);
	...
	while(gets(buf))
	{
		if(strlen(buf) == 0)
			break;
		if(m_pServerObj->SendMessagePort(buf))
		{
